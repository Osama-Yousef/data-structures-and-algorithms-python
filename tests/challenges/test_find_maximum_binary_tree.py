from data_structures_and_algorithms.challenges.find_maximum_binary_tree.find_maximum_binary_tree import BinaryTree, Node, BinarySearchTree
import pytest
...
def test_return_max_single_node():
    '''Test that binary tree with single item (just the root) WILL RETURN the root value because it cosiders as max value'''
    tree = BinaryTree(15)
...
def test_return_maxVal_of_all(tree):
    '''Test that binary tree with many nodes returns the max value between all nodes'''
